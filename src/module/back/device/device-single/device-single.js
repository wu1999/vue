    ...
    let $vue = new Vue({
        ...
        methods: {
            ...
            insertDevice:function(){
                ...
                IOT.showOverlay('保存中，请稍等...');
                IOT.getServerData(URI.DEVICE.SINGLE.ADD.SAVE,item,(ret) => {
            ...
            IOT.getServerData(URI.DEVICE.INFO.LIST,{},(ret) => {
                if (ret.code === 200) {
                    ...
                    $.each(list,function(i,v){
                        $vue.$data.infoItem.push({text:v.deviceName,value:v.deviceId})
                    });
        ...
        mounted: function () {
            this.$nextTick(() => {
                var $accountBox = $('.device-single-hook');
            
                let tableHookName = '.device-single-table-hook';
                // 查询
...

                        {
                            ...
                            events: {
                                ...
                                'click .delete': function (e, value, rowData, index) {
                                    ...
                                    layer.confirm(' &nbsp;&nbsp; &nbsp;&nbsp;是否确认删除此设备单品？', {
                                        btn: ['确定','取消']
                                    }, function(){
                                        IOT.showOverlay('保存中，请稍等...');
                                        var deviceSingleId = rowData.deviceSingleId;
                                        IOT.getServerData(URI.DEVICE.SINGLE.DELETE,{deviceSingleId:deviceSingleId},(ret) => {
                                            IOT.hideOverlay();
                                            if (ret.code === 200) {
                                                IOT.tips('保存成功！', 'success', 1000 , function () {
                                                    layer.closeAll();
